...
Route::post('create_reply_action/{$comment_id}', function($comment_id){
    $author = request('author');
    $reply_message = request('reply_message');
    $comment_id = $comment->comment_id;
    $post_id = $comment->post_id;
    $id = create_reply($comment_id, $reply_message, $author);
    if ($id) {
        return redirect("/post_detail/{$post_id}");
    } else {
        die("Error while adding comment.");
...
function create_reply($comment_id, $reply_message, $author){ 
    $sql = "insert into Reply (user_name, reply_message, comment_id, date) values (?, ?, ?, ?)";
    $current_date = date('Y-m-d');
