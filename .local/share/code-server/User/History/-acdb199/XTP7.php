...
Route::get('create_reply_action/{comments}', function($comments){
    ...
    foreach ($comments as $comment) {
        ...
        create_reply($comment, $reply_message, $author);
    }
});
...
function create_reply($comment, $reply_message, $author){ 
    $sql = "insert into Reply (user_name, comment_message, post_id, date) values (?, ?, ?, ?)";
    $current_date = date('Y-m-d');
    DB::insert($sql, array($author, $comment_message, $post_id, $current_date));
    $id = DB::getPdo()->lastInsertId();
